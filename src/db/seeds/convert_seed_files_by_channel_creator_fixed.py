#!/usr/bin/env python3
"""
Convert seed SQL files to the new schema.

Run from the same directory as these files:
...
What this fixes:
  ...
  - If channels.sql already has a creator_id column, that value is ignored and
    replaced using creator_channels.sql.
  - If channels.sql is still old-style without creator_id, it is accepted.
...
def normalize_bare_insert(sql: str, table: str, columns: list[str]) -> str:
    """
    Convert bare inserts:

      INSERT INTO table VALUES (...)

    into explicit-column inserts:

      INSERT INTO table (a, b, c) VALUES (...)

    Existing explicit-column inserts are left alone.
    """
    col_list = ", ".join(columns)

    pattern = re.compile(
        rf"\bINSERT\s+INTO\s+{re.escape(table)}\s+VALUES\b",
        flags=re.IGNORECASE,
    )

    return pattern.sub(f"INSERT INTO {table} ({col_list}) VALUES", sql)


def create_import_db() -> sqlite3.Connection:
    """
    Build an in-memory DB that can load the old seed files and the accidentally
    partially-converted channel seed.

    The final output SQL is generated explicitly afterward.
    """
...
def load_seed_files(conn: sqlite3.Connection) -> None:
    creators_sql = read_required_sql(CREATORS_IN)
    channels_sql = read_required_sql(CHANNELS_IN)
    creator_channels_sql = read_required_sql(CREATOR_CHANNELS_IN)

    creators_sql = normalize_bare_insert(
        creators_sql,
        "creators",
        ["id", "name", "description", "occupation", "education"],
    )

    # This is the critical fix. The import table has creator_id, but old bare
    # channel inserts only had the first 8 columns. Explicit channel inserts
    # that already include creator_id are left untouched.
    channels_sql = normalize_bare_insert(
        channels_sql,
        "channels",
        [
            "id",
    ...
    creator_channels_sql = normalize_bare_insert(
        creator_channels_sql,
        "creator_channels",
        ["creator_id", "channel_id"],
    )

    conn.executescript(creators_sql)
    conn.executescript(channels_sql)
    conn.executescript(creator_channels_sql)

    tags_sql = read_optional_sql(TAGS_IN)
    if tags_sql is not None:
        tags_sql = normalize_bare_insert(tags_sql, "tags", ["name"])
...
def get_channel_creator_map(conn: sqlite3.Connection) -> dict[int, int]:
    ...
    First means the first row for that channel in creator_channels.sql.
    SQLite rowid preserves insertion order for this import table.
    """
    rows = conn.execute(
        """
...
def warn_about_orphan_creator_channel_rows(conn: sqlite3.Connection) -> None:
    bad_creator_refs = conn.execute(
        """
    ...
    for (creator_id,) in bad_creator_refs:
        REPORT_LINES.append(
            f"creator_channels.sql references missing creator_id {creator_id}."
        )

    bad_channel_refs = conn.execute(
        """
    ...
    for (channel_id,) in bad_channel_refs:
        REPORT_LINES.append(
            f"creator_channels.sql references missing channel_id {channel_id}; ignored."
        )
...
def convert_channels(
    ...
    output = [
        ...
        "-- channels.creator_id was rewritten from creator_channels.sql.",
        "-- If a channel had multiple creators, the first listed creator was used.",
        "-- Any existing creator_id in channels.sql was ignored.",
        "",
    ]
    ...
    for row in rows:
        ...
        if old_creator_id is not None and int(old_creator_id) != int(creator_id):
            REPORT_LINES.append(
                f"Channel {channel_id} had existing creator_id {old_creator_id} "
                f"in channels.sql; replaced with {creator_id} from creator_channels.sql."
            )
